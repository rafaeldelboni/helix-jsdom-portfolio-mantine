goog.loadModule(function(exports) {
  ...
  function init$1(modules, domApi) {
    ...
    return function patch(oldVnode, vnode) {
      ...
      i = 0;
      for (; i < insertedVnodeQueue.length; ++i) {
        insertedVnodeQueue[i].data.hook.insert(insertedVnodeQueue[i]);
      }
      i = 0;
      for (; i < cbs.post.length; ++i) {
        cbs.post[i]();
      }
      return vnode;
    };
    ...
    for (key in oldDataset) {
      if (!dataset[key]) {
        if (d) {
          ...
          elm.removeAttribute("data-" + key.replace(CAPS_REGEX, "-$\x26").toLowerCase());
        }
      }
    }
    for (key in dataset) {
      if (oldDataset[key] !== dataset[key]) {
        if (d) {
          ...
          elm.setAttribute("data-" + key.replace(CAPS_REGEX, "-$\x26").toLowerCase(), dataset[key]);
        }
      }
    }
  }
  ...
  const htmlDomApi = {createElement, createElementNS, createTextNode, createComment, insertBefore, removeChild, appendChild, parentNode, nextSibling, tagName, setTextContent, getTextContent, isElement, isText, isComment};
  ...
  const thunk = function thunk(sel, key, fn, args) {
    ...
    return h(sel, {key:key, hook:{init, prepatch}, fn:fn, args:args});
  };
  ...
  const eventListenersModule = {create:updateEventListeners, update:updateEventListeners, destroy:updateEventListeners};
  ...
  const styleModule = {pre:forceReflow, create:updateStyle, update:updateStyle, destroy:applyDestroyStyle, remove:applyRemoveStyle};
  (function(jsx) {
  })(jsx || (jsx = {}));
  exports.array = array;
...
//# sourceMappingURL=dumdom.snabbdom.js.map
